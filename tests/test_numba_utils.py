...
@pytest.mark.parametrize("name, nb_array_mapping_get_fn", get_func_variants(nb_array_mapping_get))
def test_nb_array_mapping_get(name, nb_array_mapping_get_fn):
    ...
    assert nb_array_mapping_get_fn(keys, values, 40) == 3 # returns len(keys) if not found
...
@pytest.mark.parametrize("name2, contains_fn", get_func_variants(int_set_contains_same_dtype))
