...
def test_table_initialization(empty_db, base_schemas):
    ...
    assert len(table_a.column_ids) == 4 * 2 # 2 normal + 2 adj injected columns, times 2 for strings
...
def test_normalize_records_row_major(empty_db, base_schemas):
    ...
    records = list(table_a.normalize_records_row_major(
        {col_weight: 5.0, col_pos: [1, 2, 3]}
    ))
...
def test_normalize_records_col_major(empty_db, base_schemas):
    ...
    records = table_a.normalize_records_col_major(
        {col_weight: [10.0, 20.0]}
    )
