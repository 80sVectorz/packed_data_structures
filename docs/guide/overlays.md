...
To solve this, `packed_data_structures` encourages the use of the **Overlays Pattern**. Overlays are an optional architectural pattern where you build domain-specific wrappers around the core relational database. They expose a constrained, high-level API to your users while managing the underlying tables and schemas internally.
...
Instead of asking users to manually instantiate `TableSchema` objects and carefully wire up `ForeignKeySchema` dependencies to build adjacency lists, the `GraphOverlay` abstracts this away:
...
To the user, they simply asked for an edge layer. The overlay ensures that the database is wired up correctly under the hood.
